...
namespace cs361_lab03
{
    public class GenFunctions
    {
        ...
        /// <summary>
        /// Overloaded convert to strings function for 2D arrays.
        /// </summary>
        /// <param name="arrayOfInts">
        /// The given 2D array.
        /// </param>
        /// <returns>
        /// The 2D array of string equivilents.
        /// </returns>
        public static string[,] ConvertToStrings(int[,] arrayOfInts)
        {
            string[,] arrayOfStrings = new string[arrayOfInts.GetLength(0), arrayOfInts.GetLength(1)];

            for (int i = 0; i < arrayOfInts.GetLength(0); i++)
            {
                for (int j = 0; j < arrayOfInts.GetLength(1); j++)
                {
                    arrayOfStrings[i,j] = arrayOfInts[i,j].ToString();
                }
            }
            return arrayOfStrings;
        }
        ...
        /// </param>
        public static void PrintTable(string label, int[,] data)
        {
            string[,] dataStrings = ConvertToStrings(data);

            Console.WriteLine(label);

            int cellWidth = LongestString(dataStrings) + 3;

            PrintLine(data.GetLength(0) - 1, cellWidth);

            for (int i = 1; i < dataStrings.GetLength(0); i++)
            {
                Console.Write("|");

                for(int j = 1; j < dataStrings.GetLength(1); j++)
                {
                    Console.Write(" " + dataStrings[i,j].ToString() + WhiteSpaceFill(dataStrings[i,j].ToString(), cellWidth - 2) + " |");
                }
                Console.WriteLine("");

                PrintLine(dataStrings.GetLength(0) - 1, cellWidth);
            }
            Console.WriteLine("");
        }
    }     
